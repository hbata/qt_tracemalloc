...
class StatsManager:
    ...
    def __init__(self, window, app):
        ...
        window.connect(self.group_by, QtCore.SIGNAL("currentIndexChanged(int)"),
                       self.group_by_changed)
        window.connect(self.view, QtCore.SIGNAL("doubleClicked(const QModelIndex&)"),
                       self.double_clicked)
        window.connect(self.cumulative_checkbox,
                       QtCore.SIGNAL("stateChanged(int)"), self.change_cumulative)
        window.connect(self.snapshots.load_button,
                       QtCore.SIGNAL("clicked(bool)"), self.load_snapshots)
        window.connect(self.view.selectionModel(),
                       QtCore.SIGNAL("selectionChanged(const QItemSelection&, const QItemSelection&)"), self.selection_changed)
    ...
    def double_clicked(self, index):
        ...
        if group_by == 'filename':
            ...
            self.filters.append(tracemalloc.Filter(True,
            ...
            new_filter = tracemalloc.Filter(True,
                                            stat.traceback[0].filename,
                                            stat.traceback[0].lineno, all_frames=False)
            if self.filters:
...
class SnapshotManager:
    def __init__(self, parent):
        ...
        self.combo1.setSizePolicy(
            QtGui.QSizePolicy(QtGui.QSizePolicy.Expanding, QtGui.QSizePolicy.Minimum))
...
class SourceCodeManager:
    def __init__(self, window):
        ...
        window.connect(self.traceback_view.selectionModel(),
...
class MainWindow(QMainWindow):
    def __init__(self, app, filenames):
        ...
        self.stats.filters_label.setSizePolicy(QtGui.QSizePolicy(QtGui.QSizePolicy.Expanding,
        ...
        self.source.traceback_view.setSizePolicy(QtGui.QSizePolicy(
